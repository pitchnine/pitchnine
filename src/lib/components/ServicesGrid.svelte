  ...
  const services = [
    {
      ...
      more: 'Evidence-first review of your backlog and operating model. We de-duplicate, theme, and prioritize under clear decision rules; install cadence discipline; and return a credible 90-day plan with owners, dates, and measures.'
    },
    {
      title: 'AI Readiness',
...
<section class="py-16">
  <div class="mx-auto max-w-7xl px-6">
      ...
      {#each services as s}
        <li class="group relative isolate w-full">
          ...
          <div
            ...
            class="cursor-pointer grid w-full grid-cols-1 items-start gap-x-10 gap-y-6 p-9 text-left
                   lg:grid-cols-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 rounded-xl"
          >
            <!-- Left: title -->
            <div class="self-start">
              <h4 class="text-5xl font-semibold tracking-tight text-white sm:text-4xl">
                {s.title}
              </h4>
            </div>
    ...
    <p class="mt-8 text-center text-xs font-medium text-white/40">
      Pitch Nine is not a certification body; we install practical controls that align with your existing compliance program and industry standards.
    </p>
  </div>
</section>
