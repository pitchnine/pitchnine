...
>
  {#if title || subtitle}
    <header class="space-y-3">
      {#if title}<h2 class="inter header-text text-left mb-9">{title}</h2>{/if}
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
    ...
    <div class="col-span-1">
      ...
      <p class="mt-2 text-xs inter text-gray-500">Tip: Hold ⌘/Ctrl (desktop) to select multiple.</p>
  ...
  <div class="pt-2">
    <button
      type="submit"
      class="inter inline-flex items-center justify-center h-11 px-6 rounded-full bg-emerald-600 text-white font-semibold tracking-tight shadow-sm hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed"
      disabled={!isValid}
    >
