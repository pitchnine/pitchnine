...
<div class="w-full flex flex-col md:flex-row items-start justify-between gap-12 md:gap-32">
      
      <!-- Left column: text -->
      <div class="space-y-6 w-full md:w-1/3">
        <h2 class="inter header-form">{title}</h2>
        {#if subtitle}
          <p class="inter text-lg text-gray-300 leading-relaxed">
            {subtitle}
          </p>
        {/if}
                ...
                {#each interestsOptions as opt, i}
                  <div>
                    ...
                    >
                      ...
                      >
                        <span class="badge-check opacity-0 peer-checked:opacity-100 transition-opacity duration-150 [&_svg]:h-4 [&_svg]:w-4">
                          {@html CheckIcon}
                        </span>
                      </span>
