<script lang="ts">
  ...
  export function plotFromAnswers(answers: ('A'|'B'|'C'|'D'|null)[]) {
    ...
    const pickedStages = answers.filter(Boolean).map(a => stageByLetter[a as 'A'|'B'|'C'|'D']);
  ...
  onMount(() => {
    ...
    chart = new Chart(canvasEl, {
      ...
      options: {
        ...
        layout: { padding: 0 },
        scales: {
          x: { type: 'linear', min: 0, max: 600, grid: { display: false }, ticks: { display: false } },
          y: { type: 'linear', min: 0, max: 400, reverse: true, grid: { display: false }, ticks: { display: false } }
        },
        plugins: {
          ...
          annotation: {
            annotations: {
              intro:   { type: 'box', xMin: 25,  xMax: 155, yMin: 25, yMax: 375, backgroundColor: 'rgba(249,250,251,0.25)' },
              growth:  { type: 'box', xMin: 165, xMax: 295, yMin: 25, yMax: 375, backgroundColor: 'rgba(249,250,251,0.25)' },
              maturity:{ type: 'box', xMin: 305, xMax: 435, yMin: 25, yMax: 375, backgroundColor: 'rgba(249,250,251,0.25)' },
              decline: { type: 'box', xMin: 445, xMax: 575, yMin: 25, yMax: 375, backgroundColor: 'rgba(249,250,251,0.25)' },
            }
          }
        },
...
<canvas bind:this={canvasEl} class="w-full h-auto"></canvas>
